...
$sudah_submit = mysqli_fetch_assoc(mysqli_query($conn, "
    SELECT COUNT(*) as total FROM submissions WHERE mahasiswa_id = $mhs_id
"))['total'];

$sudah_dinilai = mysqli_fetch_assoc(mysqli_query($conn, "
    SELECT COUNT(*) as total FROM submissions WHERE mahasiswa_id = $mhs_id AND nilai IS NOT NULL
"))['total'];

$rata_nilai = mysqli_fetch_assoc(mysqli_query($conn, "
    SELECT AVG(nilai) as rata FROM submissions WHERE mahasiswa_id = $mhs_id AND nilai IS NOT NULL
"))['rata'];
...
$nilai_terbaru = mysqli_query($conn, "
    SELECT s.nilai, s.submitted_at, t.judul
    FROM submissions s
    JOIN tugas t ON s.tugas_id = t.id
    WHERE s.mahasiswa_id = $mhs_id AND s.nilai IS NOT NULL
    ORDER BY s.submitted_at DESC LIMIT 5
");

// Ambil semua kolom informasi termasuk poster, warna_bg, tipe
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .shortcut-card { background:white; border-radius:12px; padding:18px; text-align:center; text-decoration:none; color:inherit; box-shadow:0 2px 8px rgba(0,0,0,0.06); transition:transform 0.2s; border-top:3px solid transparent; }
...
<div class="main-content">
    <div class="topbar">
        <h1>Halo, <?= htmlspecialchars($_SESSION['user_nama']) ?>! 👋</h1>
    ...
    <div class="page-body">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div><h3><?= $sudah_submit ?></h3><p>Sudah Dikumpul</p></div>
            </div>
            <div class="stat-card">
                <div class="stat-icon" style="background:#fff8e1">⭐</div>
                <div><h3><?= $sudah_dinilai ?></h3><p>Sudah Dinilai</p></div>
            </div>
            <div class="stat-card">
        ...
        <div class="row-2">
            <div class="card">
                <div class="card-header">
                    <h3>⏰ Tugas Aktif (Belum Dikumpul)</h3>
                ...
                <div class="card-body">
                    ...
                    <div class="tugas-item">
                        ...
                        <div class="countdown <?= $kelas_c ?>">
                            ...
                            <?php else: ?> — <?= $hari_sisa ?> hari lagi
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php endwhile; ?>
                    <?php if (!$ada): ?>
                    <p style="text-align:center;color:#aaa;padding:20px 0">Semua tugas sudah dikumpulkan ✅</p>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <?php
                    $ada_nilai = false;
                    while ($n = mysqli_fetch_assoc($nilai_terbaru)):
                        $ada_nilai = true;
                        $warna_nilai = $n['nilai'] >= 80 ? '#27ae60' : ($n['nilai'] >= 60 ? '#f39c12' : '#e74c3c');
                    ?>
                    <div class="tugas-item">
                        <div style="display:flex;justify-content:space-between;align-items:center">
                            <div>
                                <h4><?= htmlspecialchars($n['judul']) ?></h4>
                                <p><?= date('d M Y', strtotime($n['submitted_at'])) ?></p>
                            </div>
                            <div style="font-size:22px;font-weight:700;color:<?= $warna_nilai ?>"><?= $n['nilai'] ?></div>
                        </div>
                    </div>
                    <?php endwhile; ?>
                    <?php if (!$ada_nilai): ?>
                    <p style="text-align:center;color:#aaa;padding:20px 0">Belum ada nilai</p>
                    <?php endif; ?>
                </div>
            </div>
        </div>
                            ...
                            ?>
                                ...
                                <div class="poster-img-wrap">
                                    ...
                                    <div class="poster-img-overlay">
                                        ...
                                        <?php if ($i['isi']): ?>
                                        <p style="color:#ffffff;"><?= htmlspecialchars(substr($i['isi'], 0, 100)) ?><?= strlen($i['isi'])>100?'...':'' ?></p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            <?php else: ?>
